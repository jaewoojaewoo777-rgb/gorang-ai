...
export function getAuthUrl({ forceSelect = false } = {}) {
  const client = getOAuthClient()
  return client.generateAuthUrl({
    access_type: 'offline',
    scope: SCOPES,
    // forceSelect: 계정/채널 선택 화면을 강제로 띄움 (유튜브 채널 잘못 연결됐을 때 다시 고르기 위함)
    prompt: forceSelect ? 'select_account consent' : 'consent',
  })
}
...
export async function uploadYouTubeVideo({ accessToken, title, description, videoBuffer, videoUrl, mimeType, isShorts, tags }) {
  ...
  const metadata = {
    snippet: {
      ...
      tags: tags && tags.length > 0
        ? (isShorts ? ['Shorts', ...tags] : tags)
        : (isShorts ? ['Shorts', '제주', '카페', '펜션'] : ['제주', '카페', '펜션']),
    },
